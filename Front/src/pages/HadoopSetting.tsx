...
const HadoopSetting: React.FC = () => {
    ...
    const handleIntegratedQuery = async () => {
        ...
        try {
            const [kafkaResponse, mysqlResponse] = await Promise.all([
                axios.post('http://localhost:8000/get-kafka-clusters', {
                    access_key_id: accessKey,
                    access_key_secret: secretKey,
                }),
                axios.post('http://localhost:8000/get-instance-groups', {
                    access_key_id: accessKey,
                    access_key_secret: secretKey,
                })
            ]);
    ...
    const handleInstanceSelect = async (instanceName: string) => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/get-instance-endpoints', {
                access_key_id: accessKey,
                access_key_secret: secretKey,
