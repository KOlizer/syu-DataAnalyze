...
# ----------------------------------------------------------------------
# 0. 네트워크 안정성과 환경 준비를 위해 약간 지연 (필요 시 조정)
# ----------------------------------------------------------------------
echo "네트워크 및 환경 준비를 위해 잠시 대기합니다..."
sleep 3

# ----------------------------------------------------------------------
# 1. 일반 사용자 홈 디렉토리 설정 (예: ubuntu)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 2. 환경 변수 적용
#    setup_initial.sh에서 ~/.bashrc에 추가한 환경 변수를 소스합니다.
# ----------------------------------------------------------------------
if [ -f "$USER_HOME/.bashrc" ]; then
    echo "환경 변수 적용: $USER_HOME/.bashrc를 소스합니다."
    source "$USER_HOME/.bashrc"
else
    echo "경고: $USER_HOME/.bashrc 파일을 찾을 수 없습니다. 환경 변수가 제대로 설정되었는지 확인하세요."
fi

###############################################################################
# 3. config.yml 파일 생성 (~/syu-DataAnalyze/TrafficGenerator)
###############################################################################
echo "========================================"
...
# 파일 무결성 체크: Go tarball은 약 100MB 이상이어야 함 (최소 50MB로 체크)
MIN_SIZE=52428800
...
###############################################################################
# 6. 로그 파일 및 디렉토리 권한 수정 (REST API VM1, VM2)
###############################################################################
echo "========================================"
